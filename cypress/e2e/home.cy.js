...
describe('Home Page Tests', function () {
  ...
  beforeEach(function () {
    ...
    // *** FINAL FIX FOR INTERCEPT PATTERN ***
    // Use the full internal Docker network URL for the backend API
    // This matches how your frontend's VITE_API_URL is configured to talk to the backend service.
    cy.intercept('GET', 'http://backend:8000/api/recipes/categories/**').as('getCategories');
    cy.intercept('GET', 'http://backend:8000/api/recipes/latest/**').as('getLatestRecipes');
    // *** END FINAL FIX ***

    // Re-add cy.wait() to ensure categories and recipes are fetched before proceeding
  ...
  it('3. Displays category filter badges and allows selection', () => {
    ...
    cy.getDataTest('badge-label').contains('Comida').should('be.visible'); // This should now pass!
...
//   beforeEach(function () {
//     cy.setupHomePage('ana456', 'testpass456');
//     cy.intercept('GET', '/api/recipes/categories/**').as('getCategories').debug();
//     cy.intercept('GET', '/api/recipes/latest/**').as('getLatestRecipes').debug();
